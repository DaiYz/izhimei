...
//
//  });
  BOOL install = [[UMSocialManager defaultManager] isInstall:[self platformType:platform]];
     
     if (install) {
       completion([[NSArray alloc] initWithObjects:@"true", nil]);
     }else{
       completion([[NSArray alloc] initWithObjects:@"false", nil]);
     }
}
